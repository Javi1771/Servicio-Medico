    ...
    try {
      ...
      if (claveusuario !== null) {
        ...
        await pool.request()
          ...
          .input("idProveedor", sql.Int, insertedId)
          .query(`
          ...
          `);
        console.log("Actividad 'Agregó un nuevo proveedor' registrada en ActividadUsuarios.");
      } else {
        console.log("No se pudo registrar la actividad: falta claveusuario.");
      }

      return res.status(201).json({ message: 'Proveedor agregado exitosamente', claveproveedor: insertedId });
    } catch (error) {
