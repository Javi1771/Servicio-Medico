...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!selectResult.recordset.length) {
      return res.status(404).json({ message: "Proveedor no encontrado después de la actualización" });
    }
    ...
    if (claveusuario !== null) {
      ...
      await pool.request()
        ...
        `);
      console.log("Actividad 'Eliminó un proveedor' registrada en ActividadUsuarios.");
    } else {
      console.log("No se pudo registrar la actividad: falta claveusuario.");
    }

    return res.status(200).json({ message: "Proveedor desactivado correctamente" });
  } catch (error) {
