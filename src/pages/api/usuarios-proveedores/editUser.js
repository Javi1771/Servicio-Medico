...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!selectResult.recordset.length) {
      return res.status(404).json({ message: 'Proveedor no encontrado tras la actualización.' });
    }
    ...
    return res.status(500).json({ message: 'Error al actualizar el proveedor', error: error.message });
  }
}
