    ...
    try {
      ...
      if (claveusuario !== null) {
        ...
        await pool.request()
          ...
          .input("idEnfCronica", sql.Int, insertedId)
          .query(`
          ...
          `);
        console.log("Actividad 'Agregó una enfermedad crónica' registrada en ActividadUsuarios.");
      } else {
      ...
      if (error.message.includes('duplicate key')) {
        return res.status(409).json({ message: 'La enfermedad crónica ya existe.' });
      } else {
        return res.status(500).json({ message: 'Error al agregar la enfermedad crónica.', error: error.message });
      }
    }
  } else {
