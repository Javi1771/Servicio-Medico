...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      if (!hasTime) {
        console.log('⚠️ [guardarConsulta] Hora faltante en fecha:', consultaData.fechaconsulta);
      ...
      const result = await pool
        ...
        .input("clavepaciente", sql.NVarChar(15), consultaData.clavepaciente || null)
        ...
        .input(
          "departamento",
          sql.NChar(200),
          consultaData.departamento || null
        )
        .input(
      ...
      console.log(`✅ [guardarConsulta] Insert en consultas exitoso. claveConsulta=${claveConsulta}`);
      ...
      if (claveProveedorNum === 610) {
        ...
        console.log(`⛔ [guardarConsulta] claveProveedorNum ${claveProveedorNum} no es 610; no inserta en costos`);
      }
      ...
      if (claveusuarioInt !== null) {
        console.log(`➡️ [guardarConsulta] Insertando en ActividadUsuarios para user ${claveusuarioInt}`);
        const ip =
      ...
      return res.status(200).json({ success: true, message: "Consulta guardada correctamente.", claveConsulta });
    } catch (error) {
