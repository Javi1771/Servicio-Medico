...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Validar que se obtuvo quien capturó
    if (!quienCapturo) {
      console.error("Error: No se encontró la clave del usuario en la cookie");
      return res.status(400).json({ error: "No se encontró la clave del usuario en la cookie" });
    }
    ...
    // Obtener la fecha actual en el formato correcto para SQL Server
    const now = new Date();
    const fechaActual = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")} ${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}:${String(now.getSeconds()).padStart(2, "0")}.${String(now.getMilliseconds()).padStart(3, "0")}`;
    ...
    await request
      ...
      .query(`
        INSERT INTO incapacidades (fecha, fechainicio, fechafin, nomina, nombrepaciente, departamento, observaciones, edad, claveconsulta, claveMedico, estatus, cancelo, quiencapturo)
        VALUES (@fecha, @fechainicio, @fechafin, @nomina, @nombrepaciente, @departamento, @observaciones, @edad, @claveconsulta, @claveMedico, @estatus, @cancelo, @quiencapturo)
      `);
