...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      // /public/Beneficiarios/Documentos/ActasMatrimonioManual/{numNomina}/{parentescoTexto}/{nombre}_{aPaterno}_{aMaterno}/
      ...
      const fileName = `acta_matrimonio_manual_${Date.now()}${path.extname(
      ...
      const finalURL = `${process.env.NEXT_PUBLIC_BASE_URL}/Beneficiarios/Documentos/ActasMatrimonioManual/${numNomina}/${parentescoTexto}/${folderName}/${fileName}`;

      return res.status(200).json({
        url: finalURL,
        message: "Acta de Matrimonio subida correctamente al servidor propio.",
      });
    } catch (error) {
