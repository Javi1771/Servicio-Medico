...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      // /public/Beneficiarios/Documentos/ActasNacimiento/{numNomina}/{parentescoTexto}/{nombre}_{aPaterno}_{aMaterno}/
      ...
      const fileName = `acta_nacimiento_${Date.now()}${path.extname(
      ...
      const finalURL = `${process.env.NEXT_PUBLIC_BASE_URL}/Beneficiarios/Documentos/ActasNacimiento/${numNomina}/${parentescoTexto}/${folderName}/${fileName}`;

      return res.status(200).json({
        url: finalURL,
        message:
          "Acta de Nacimiento subida correctamente al servidor propio.",
      });
    } catch (error) {
