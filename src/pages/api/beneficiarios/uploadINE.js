...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    // Validar la presencia de los campos
    if (!file) {
    ...
    if (!nombre || !aPaterno || !aMaterno || !parentesco) {
      ...
      return res.status(400).json({
        error: "El nombre, apellido paterno, apellido materno y el parentesco son obligatorios.",
      });
    }

    // Traducir el ID de parentesco al texto
    const parentescoTexto = PARENTESCO_MAP[parentesco] || "Desconocido";
    // Construir el nombre de la carpeta a partir del nombre completo (limpiar espacios)
    const folderName = `${nombre.trim()}_${aPaterno.trim()}_${aMaterno.trim()}`.replace(/\s+/g, "_");

    try {
      // Construir la carpeta destino:
      // /public/Beneficiarios/Documentos/INE/{numNomina}/{parentescoTexto}/{folderName}/
      ...
      const finalURL = `${process.env.NEXT_PUBLIC_BASE_URL}/Beneficiarios/Documentos/INE/${numNomina}/${parentescoTexto}/${folderName}/${fileName}`;

      return res.status(200).json({
        url: finalURL,
        message: "INE subida correctamente al servidor propio.",
      });
    } catch (error) {
