...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    // Construir el nombre de la carpeta a partir del nombre completo (sin espacios)
    const folderName = `${nombre.trim()}_${aPaterno.trim()}_${aMaterno.trim()}`.replace(/\s+/g, "_");

    try {
      // Construir la carpeta destino:
      // /public/Beneficiarios/Documentos/ConstanciasEstudios/{numNomina}/{parentescoTexto}/{folderName}/
      ...
      const finalURL = `${process.env.NEXT_PUBLIC_BASE_URL}/Beneficiarios/Documentos/ConstanciasEstudios/${numNomina}/${parentescoTexto}/${folderName}/${fileName}`;
