...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    // Construir el nombre de la carpeta a partir del nombre completo (eliminando espacios)
    ...
    try {
      ...
      // /public/Beneficiarios/Documentos/Curps/{numNomina}/{parentescoTexto}/{folderName}/
      ...
      const fileName = `curp_${Date.now()}${path.extname(
      ...
      const finalURL = `${process.env.NEXT_PUBLIC_BASE_URL}/Beneficiarios/Documentos/Curps/${numNomina}/${parentescoTexto}/${folderName}/${fileName}`;

      return res.status(200).json({
        url: finalURL,
        message: "CURP subida correctamente al servidor propio.",
      });
    } catch (error) {
      console.error("Error al subir el archivo CURP:", error);
      return res
        .status(500)
