...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    // Asegurarse de que no sean arrays
    ...
    try {
      ...
      // /public/Beneficiarios/Documentos/ActasConcubinatoManual/{numNomina}/{parentescoTexto}/{nombre}_{aPaterno}_{aMaterno}/
      const folderName = `${nombre.trim()}_${aPaterno.trim()}_${aMaterno.trim()}`
        .replace(/\s+/g, "_");
      ...
      const fileName = `acta_concubinato_manual_${Date.now()}${path.extname(
      ...
      const finalURL = `${process.env.NEXT_PUBLIC_BASE_URL}/Beneficiarios/Documentos/ActasConcubinatoManual/${numNomina}/${parentescoTexto}/${folderName}/${fileName}`;

      return res.status(200).json({
        url: finalURL,
        message:
          "Archivo de Acta de Concubinato Manual subida correctamente al servidor propio.",
      });
    } catch (error) {
