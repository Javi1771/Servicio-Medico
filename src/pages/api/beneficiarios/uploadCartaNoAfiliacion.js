...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      // /public/Beneficiarios/Documentos/CartasNoAfiliacion/{numNomina}/{parentescoTexto}/{nombre}_{aPaterno}_{aMaterno}/
      ...
      const fileName = `carta_no_afiliacion_${Date.now()}${path.extname(
      ...
      const finalURL = `${process.env.NEXT_PUBLIC_BASE_URL}/Beneficiarios/Documentos/CartasNoAfiliacion/${numNomina}/${parentescoTexto}/${folderName}/${fileName}`;

      return res.status(200).json({
        url: finalURL,
        message: "Carta de No Afiliación subida correctamente al servidor propio.",
      });
    } catch (error) {
