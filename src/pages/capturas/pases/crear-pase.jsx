...
const CrearPase = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`/api/especialidades/obtenerConsultaEspecialidad?folio=${folio}`);
      ...
      MySwal.fire({
        icon: "error",
        title: "<span style='color: #ff6b6b; font-weight: bold; font-size: 1.5em;'>⚠️ Error al cargar datos</span>",
        ...
        customClass: {
          popup: "border border-red-500 shadow-[0px_0px_20px_5px_rgba(255,107,107,0.8)] rounded-lg",
        },
      });
    }
  }, [folio]);
  ...
  const handleGuardar = async () => {
    if (!selectedEspecialista || !fechaCita) {
      MySwal.fire({
        icon: "warning",
        title: "<span style='color: #ffbb33; font-weight: bold; font-size: 1.5em;'>⚠️ Faltan Datos</span>",
        ...
        customClass: {
          popup: "border border-yellow-500 shadow-[0px_0px_20px_5px_rgba(255,187,51,0.7)] rounded-lg",
        },
      });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        MySwal.fire({
          icon: "success",
          title: "<span style='color: #00e676; font-weight: bold; font-size: 1.5em;'>✔️ Pase Guardado</span>",
          ...
          confirmButtonText: "<span style='color: #fff; font-weight: bold;'>Aceptar</span>",
          customClass: {
            popup: "border border-green-500 shadow-[0px_0px_20px_5px_rgba(0,230,118,0.7)] rounded-lg",
          },
        });
        router.push("/pases");
      } else {
      ...
      MySwal.fire({
        icon: "error",
        title: "<span style='color: #ff6b6b; font-weight: bold; font-size: 1.5em;'>❌ Error al Guardar</span>",
    ...
    <div className="relative min-h-screen bg-gradient-to-br from-black via-blue-900 to-teal-500 text-white py-10 px-12">
          ...
          <div className="grid grid-cols-2 gap-6 flex-1">
            ...
            <div className="flex items-center bg-gray-800 p-5 rounded-xl shadow-md border-2 border-transparent hover:border-teal-400 transition-all duration-300">
              ...
              <div>
                ...
                <p className="text-white">{data.paciente?.nombrepaciente || "N/A"}</p>
              </div>
            </div>
            ...
            <div className="flex items-center bg-gray-800 p-5 rounded-xl shadow-md border-2 border-transparent hover:border-teal-400 transition-all duration-300">
              ...
              <div>
                ...
                <p className="text-white">{data.paciente?.clavenomina || "N/A"}</p>
              </div>
            </div>
      ...
      <div className="bg-gray-900 p-8 rounded-3xl shadow-2xl">
        <div className="grid grid-cols-2 gap-8 mb-8">
          <div>
            <label className="block text-teal-300 font-semibold mb-2">Especialidad:</label>
            <p className="bg-gray-700 p-4 rounded-md">{data.especialidad?.especialidad || "Seleccione una especialidad"}</p>
          </div>
          <div>
            <label className="block text-teal-300 font-semibold mb-2">Especialista:</label>
            <select
              className="w-full p-4 rounded-md bg-gray-800 text-white"
              onChange={(e) => setSelectedEspecialista(JSON.parse(e.target.value))}
            >
