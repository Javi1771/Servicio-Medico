              ...
              return (
                <motion.div
                  ...
                  className={`relative rounded-2xl p-6 shadow-lg transition-all duration-300 overflow-hidden backdrop-blur-lg 
                    ${isVencida ? "bg-red-900/70 border border-red-500 hover:shadow-[0px_0px_30px_10px_rgba(255,0,0,0.6)]"
                      : isFutura ? "bg-blue-900/70 border border-blue-500 hover:shadow-[0px_0px_30px_10px_rgba(0,200,255,0.6)]"
                ...
                >
                  ...
                  <div
                    className={`absolute inset-0 bg-gradient-to-br opacity-10 hover:opacity-40 transition-all duration-500 
                      ${isVencida ? "from-red-500/40" : isFutura ? "from-blue-500/40" : "from-cyan-500/40"}`}
                  ></div>

                  {/* 🏥 Nombre del paciente */}
                  <h2
                    className={`text-3xl font-bold flex items-center gap-2 
                      ${isVencida ? "text-red-300" : isFutura ? "text-blue-400" : "text-cyan-400"}`}
                  >
...
{isVencida && (
  <motion.div className="absolute top-3 right-3 flex items-center gap-2 bg-red-600/40 text-white text-xs px-3 py-2 rounded-full shadow-lg backdrop-blur-xl">
    ...
    <span>Incapacidad vencida hace {incapacidad.alerta} días</span>
  </motion.div>
)}
{isFutura && (
